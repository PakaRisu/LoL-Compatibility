...
function riotGet(apiUrl) {
  return new Promise(function(resolve, reject) {
    ...
    if (!apiKey) return reject(new Error('RIOT_API_KEY not set.'));
...
module.exports = function(req, res) {
  ...
  if (!puuid || !region) {
    res.status(400).json({ error: 'Missing: puuid, region' });
    return;
  }
  const apiUrl = 'https://' + region + '.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/' + puuid;
